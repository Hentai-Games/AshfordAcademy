init:
    if persistent.mod_disable_original_events == False:
        ...
        $ event("library32", "act == 'library' and morale < 15", event.choose_one('library'))
...
label library32:
    ...
    $ topic = random_topic("academics")
    if renpy.random.random() > 0.5:
        "I have never before seen such a lack of interest in [topic]."
    else:
        pov "Hey girl, turn that frown upside down!"
        if behavior > 20:
            girl "...No.\nAnd please let me study.\nAlone."
        else:
            girl "Hey principal, go fuck yourself."
            $ behavior -= 1
