init:
    if persistent.mod_disable_original_events == False:
        ...
        $ event("bath5", "act == 'bath' and inhibition <= 75 and deviance >= 5", event.choose_one('bath'))
        $ event("bath6", "act == 'bath' and inhibition <= 75 and deviance >= 15", event.choose_one('bath'))
        ...
        $ event("bath12", "act == 'bath' and deviance > 20 and new_game_plus", event.choose_one('bath'))
        ...
        $ event("bath31", "act == 'bath' and inhibition <= 90 and inhibition > 50", event.choose_one('bath'))
...
label bath2:
    ...
    $ randTxt = renpy.random.choice([
        ...
        "As you walk into the bath you catch a glimpse of some girls horsing around."
        ])
    "[randTxt]"
    return
...
label bath6:
    ...
    if deviance > 25 or inhibition < 70:
        girl "Ahh, please..."
...
label bath31:
    if renpy.random.randint(1,2) == 1:
        scene bath31-1 with fade
        pov "Well equipped I see."
        girl "*blush*"
    else:
        scene bath31-2 with fade
        girl "Phuhuhuh.... Sooo~ relaxing!"
    return
