init:
    if persistent.mod_disable_original_events == False:
        $ event("gym1", "act == 'gym' and athletics < 25", event.choose_one('gym'))
        $ event("gym2", "act == 'gym' and inhibition > 75", event.choose_one('gym'))
        $ event("gym3", "act == 'gym' and inhibition > 85", event.choose_one('gym'))
        $ event("gym4", "act == 'gym'", event.choose_one('gym'))
        $ event("gym5", "act == 'gym' and inhibition > 75", event.choose_one('gym'))
        $ event("gym6", "act == 'gym' and athletics > 25", event.choose_one('gym'))
        $ event("gym7", "act == 'gym' and inhibition > 85", event.choose_one('gym'))
        $ event("gym8", "act == 'gym' and athletics < 20", event.choose_one('gym'))
        $ event("gym9", "act == 'gym' and inhibition < 75", event.choose_one('gym'))
        $ event("gym10", "act == 'gym' and inhibition < 90", event.choose_one('gym'))
        $ event("gym11", "act == 'gym' and inhibition < 60 and deviance > 40", event.choose_one('gym'))
        $ event("gym12", "act == 'gym' and deviance > 5 and behavior < 15", event.choose_one('gym'))
        $ event("gym13", "act == 'gym' and inhibition < 80", event.choose_one('gym'))
        $ event("gym14", "act == 'gym' and deviance > 30", event.choose_one('gym'))
        $ event("gym15", "act == 'gym' and deviance > 10", event.choose_one('gym'))
        $ event("gym16", "act == 'gym' and inhibition > 75", event.choose_one('gym'))
        $ event("gym17", "act == 'gym' and deviance > 25", event.choose_one('gym'))
        $ event("gym18", "act == 'gym' and deviance > 15 and inhibition < 80", event.choose_one('gym'))
        $ event("gym19", "act == 'gym' and inhibition < 80", event.choose_one('gym'))
        $ event("gym20", "act == 'gym' and deviance > 20", event.choose_one('gym'))
        $ event("gym21", "act == 'gym' and athletics < 40", event.choose_one('gym'))
        $ event("gym22", "act == 'gym' and inhibition > 70 and inhibition < 95", event.choose_one('gym'))
        $ event("gym23", "act == 'gym' and deviance > 15", event.once(), event.depends("class41"), event.choose_one('gym', 3))  #WD: Stop Dup Events# priority=20)
        $ event("gym24", "act == 'gym' and athletics < 50",event.choose_one('gym'))                                             #Ash: Yes yes, I will!
        $ event("gym25", "act == 'gym' and total_days > 50", event.once(), event.choose_one('gym', 3))                          #WD: Stop Dup Events# priority=20)
        $ event("gym26", "act == 'gym' and pda_rule_level >= 5 and behavior < 50", event.once(), event.choose_one('gym', 3))    #WD: Stop Dup Events# priority=20)
        $ event("gym27", "act == 'gym' and inhibition < 5 and deviance > 95", event.choose_one('gym'))                          #Ash: I heard you the first time sir, I promise.
        $ event("gym28", "act == 'gym' and inhibition < 85", event.choose_one('gym'))
        $ event("gym28", "act == 'gym' and inhibition < 95 and inhibition > 25 and deviance > 0", event.choose_one('gym'))
...
label gym8:
    ...
    $ renpy.show("gym8_"+randImg)
    with fade
    $ randTxt = renpy.random.choice([
        "Don't forget to stretch those muscles!",
        "Flexibility is just as important as strength.",
        "Dynamic warm-up prior to working out is key to a fit body.",
        "Flexible and agile muscles makes a great body."
        ])
    "[randTxt]"
    $ athletics += 1
    return
