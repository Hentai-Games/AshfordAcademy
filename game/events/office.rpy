init:
    if persistent.mod_disable_original_events == False:
        $ event("office1", "act == 'office' and morale > 30", event.choose_one('office'))
        $ event("office2", "act == 'office' and morale < 35", event.choose_one('office'))
        $ event("office3", "act == 'office' and deviance >= 45 and pda_rules == 'pda_expulsion'", event.once(), event.choose_one('office', 2))    #WD: Stop Dup Events# priority=20)
        $ event("office4", "act == 'office'", event.choose_one('office'))
        $ event("office5", "act == 'office'", event.choose_one('office'))
        $ event("office6", "act == 'office'", event.choose_one('office'))
        $ event("office7", "act == 'office'", event.choose_one('office'))
        $ event("office8", "act == 'office' and morale > 30", event.choose_one('office'))
        $ event("office9", "act == 'office' and pda_rule_level > 2", event.choose_one('office'))
        $ event("office10", "act == 'office' and morale > 30", event.choose_one('office'))
        $ event("office11", "act == 'office' and deviance > 20 and inhibition < 90", event.choose_one('office'))
        $ event("office12", "act == 'office' and behavior < 30", event.choose_one('office'))
        $ event("office13", "act == 'office' and deviance > 45 and behavior < 30", event.choose_one('office'))
        $ event("office14", "act == 'office' and behavior < 30", event.choose_one('office'))
        $ event("office15", "act == 'office' and deviance > 30 and inhibition < 70", event.choose_one('office'))
        $ event("office16", "act == 'office' and morale > 30", event.choose_one('office'))
        $ event("office17", "act == 'office' and deviance < 5 ", event.choose_one('office'))
        $ event("office18", "act == 'office' and deviance > 40 and inhibition < 85", event.choose_one('office'))
        $ event("office19", "act == 'office' and deviance > 50 and inhibition > 60", event.choose_one('office'))
        $ event("office20", "act == 'office' and deviance > 75 and inhibition < 50", event.choose_one('office'))
        $ event("office21", "act == 'office' and deviance > 75 and inhibition < 60 and evil_points > 0", event.choose_one('office'))
        $ event("office22", "act == 'office' and morale > 50", event.choose_one('office'))
        # Image lost?
        #$ event("office23", "act == 'office' and deviance > 20 and class15_sex == True", event.choose_one('office'), event.depends("class15"))
        $ event("office23", "act == 'office' and deviance > 50", event.choose_one('office'))
        $ event("office24", "act == 'office' and deviance > 50 and inhibition < 50", event.choose_one('office'))

label office1:
    if deviance > 25 and inhibition < 90:
        scene office1-4 with fade
        "Teacher" "Mhmm...\nOh god, [povFirstName]! Harder...\nAh, ahh!"
        "Working with her {i}always{/i} puts a smile on my face."
    elif deviance > 15:
        scene office1-3 with fade
        "Teacher" "Ah!\nAw, [povName]!"
        pov "You have such big and beautiful eyes... And breast. You have huge and beautiful breasts!"
        "Teacher" "*giggle* Mhmm...."
    elif deviance > 15:
        scene office1-2 with fade
        "Working with her {i}always{/i} puts a smile on my face."
    elif deviance > 15:
        scene office1-1 with fade
...
label office20:
    ...
    scene office20 with fade
    "Whats the difference between counselling and cuddling? Let's find out!"
    $ deviance += 1
    return
...
label office23:
    ...
    if behavior < deviance:
        "Teacher" "Who's been a bad girl, huh?"
        girl "Ahh... *gasp* Mhmm... I-I've... been ah! b-bad... AH! Not my nipples!"
        "Teacher" "Bad girls like you are not allowed to talk back. You are here to learn how to behave!"
        girl "Mhmm, yeah-h, teach me... AH! Mhmm, Teach me harder! *pant*"
        $ behavior += 1
    else:
        girl "Ah! *pant* harder! Mhmm... Fuck me! Ahh..."
        "Teacher" "Don't you worry, I will fuck your tight little cunt over and over til' you can't even walk!"
        girl "Mhmm, yes-AH! Teacher, please-Mhmm-please-AH! Oh, god! *pant*"
        $ deviance += 1
    return
...
label office24:
    if renpy.random.randint(1,2) == 1:
        scene office24-1 with fade
    else:
        scene office24-2 with fade
    $ randTxt = renpy.random.choice([
        "Student-teacher relations is evolving by the minute.",
        "A hands-on approach on private tutoring generates the highest teacher/student satisfaction."
        ])
    "[randTxt]"
    return
